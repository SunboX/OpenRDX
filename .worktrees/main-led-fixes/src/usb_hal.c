...
{
    /* USB HAL configure endpts; persistent state is carried in USB device state, USB endpoint enable register and related record fields. */
    ...
    if (usb_device_state != 4) {
        ...
        if (0x108a < usb_core_revision) {
            mww_process_command_word_channel_mww_command_channel(0, 0x20009, 0, 0);
        }
        core_process_descriptor_component_context(1, *(uint16_t *)(&usb_endpoint_one_max_packet_size_by_speed + usb_device_state * 2), 0);
        core_process_descriptor_component_context(0x81, *(uint16_t *)(&usb_endpoint_one_max_packet_size_by_speed + usb_device_state * 2), 1);
        ...
        if (usb_device_state != 0) {
            ...
            if (core_descriptor_queue_context == 1) {
                if (usb_device_state == 3) {
                    endpoint_bank_count = 2;
                }
                core_process_channel_completion_usb_transfer_state(2, *(uint16_t *)(&usb_endpoint_two_max_packet_size_by_speed + usb_device_state * 2), 1, 0);
                core_process_mww_process_command(2);
                core_process_channel_completion_usb_transfer_state(0x82, *(uint16_t *)(&usb_endpoint_two_max_packet_size_by_speed + usb_device_state * 2), endpoint_bank_count, 2);
                core_process_mww_process_command(0x82);
                controller_value = 0x3f;
            }
            core_process_channel_completion_usb_transfer_state(3, *(uint16_t *)(&usb_endpoint_three_max_packet_size_by_speed + usb_device_state * 2), endpoint_bank_count, 0);
            core_process_mww_process_command(3);
            core_process_channel_completion_usb_transfer_state(0x83, *(uint16_t *)(&usb_endpoint_three_max_packet_size_by_speed + usb_device_state * 2), endpoint_bank_count, 3);
