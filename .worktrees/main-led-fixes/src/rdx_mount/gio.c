 ...
 *
 * Logical output 10 maps to selector 3 with active-high polarity.  States 1/3
 * write logical zero and state 6 writes logical one before starting PWM1.
 ...
 *
 * State-driven code uses gio_rdx_motor_control_set_high() directly because
 * states 1/3 run PWM1 with GPIO3 low while state 6 runs PWM1 with GPIO3 high.
...
void gio_init(void)
{
     ...
     * pattern uses that level, while state 6 deliberately retains it; the pin is
