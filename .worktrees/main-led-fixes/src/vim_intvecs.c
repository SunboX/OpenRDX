        ...
        if (handler_status == 0) {
            ...
            if ((ahci_port_status & 0xf) != 4) {
                if (interrupt_cause == 0xf) {
                    if (interrupt_state_005 == 0) {
                        /* Snapshot interrupt state 007 before decoding its status or capability fields. */
                        interrupt_state_007 = RTIFRC1_REG_OFF;
                    }
                    interrupt_state_005 = interrupt_state_005 + 1;
                    if (4 < interrupt_state_005) {
                        interrupt_state_005 = 0;
                        /* Snapshot the fixed-address register before decoding its status or capability fields. */
                        handler_status = RTIFRC1_REG_OFF;
                        if ((uint32_t)(handler_status - interrupt_state_007) < 0x32) {
                            ...
                            interrupt_state_006 = interrupt_state_006 + 1;
                            ahci_get_TFD_info();
...

{
    ...
    core_forward_interrupt_transfer_context(10);
...
{
    /* Overflow0 isr; persistent state is carried in RTI counter zero callback. */
    rti_counter_zero_callback = rti_counter_zero_callback + 1;
...
{
    /* Overflow1 isr; persistent state is carried in RTI counter one callback. */
    rti_counter_one_callback = rti_counter_one_callback + 1;
