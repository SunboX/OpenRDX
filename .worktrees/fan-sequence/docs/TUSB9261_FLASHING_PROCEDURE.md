...
## Fan-pulse programming and hardware result

The `test/eject-button-fan-pulse` branch was built on Windows with TI ARM CGT
5.2.5. The linked output contained 472 firmware bytes through address
`0x08000297`. A continuous 664-byte FlashBurner image was generated with 192
gap bytes filled with `0xFF`; its SHA-256 was
`49E8F330249F3F7634671AB15B2E94A2F0340EE550CE7318A7FFE90946416ED8`.
Independent parsing found no changed or missing firmware bytes, no non-`0xFF`
fill bytes, and no address discontinuities.

The read-only FlashBurner probe reported one bootloader instance, the driver
present, and the SPI flash present. Programming instance zero returned exit
code zero and `Programming Succeeded`. After a USB power cycle with J7 open,
the operator confirmed that an eject-button press successfully operated the
physical fan. The firmware requests a one-second full-duty PWM0 pulse; that
