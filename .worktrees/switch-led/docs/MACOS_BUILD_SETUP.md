...
This guide describes the setup that has been verified on an Apple Silicon Mac
with a current macOS release. It builds the firmware with the official Windows
TI ARM Code Generation Tools (CGT) 5.2.9 under Wine.
...
Rosetta 2 do not run 32-bit Intel applications. The Windows 5.2.9 compiler is
also 32-bit, but Wine 11 can run it on Apple Silicon.

The project still uses TI's legacy `ti_arm9_abi`. Do not replace TI CGT with
...
Verify that the launcher reaches the expected compiler and that the legacy ABI
is accepted:
...
A successful clean build creates 30 object files and these artifacts:
...
## Windows build remains unchanged
...
are not safe for this legacy toolchain.
