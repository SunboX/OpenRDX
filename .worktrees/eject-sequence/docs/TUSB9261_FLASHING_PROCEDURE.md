...
### Failed physical motor test and diagnosis

After power-cycling with J7 open, pressing the eject button did not spin the
motor. This is a failed physical validation of the branch behavior; the earlier
successful FlashBurner result validates the programming transaction but not
the firmware's GPIO/PWM assumptions.

A comparison with the raw Ghidra export and TI reference register definitions
found two discrepancies in the minimized test:

1. The production initialization writes PWM configuration `0x12`, consisting
   of phase-1 output level bit `0x10` plus continuous-mode bit `0x02`. Its idle
   path clears mask `0x23`, which preserves bit `0x10`; the motor-start path
   then sets bit `0x02`, restoring `0x12`. The minimized branch instead clears
   mask `0x23` from the reset value and later sets only `0x02`. If the reset
   value is zero, PWM1 remains configured with a low phase-1 output even when
   its duration equals the full period. This is the strongest current
   explanation for a motionless motor.
2. One original entry path first sets logical output 5 low before driving
   logical output 10 low and applying 100 percent to logical output 13.
   Logical output 5 maps to physical selector 11, the TUSB9261
   `SPI_CS2/GPIO11` pin. The minimized branch neither configures nor drives
   this pin. Its exact board-level role has not yet been confirmed, so it must
   not be added to the test solely from its inferred name.

The original binary table at `0x0800E408` contains the relevant mappings:

| Logical index | Physical selector | Flags | Observed original use |
| ---: | ---: | ---: | --- |
| 1 | 1 (`GPIO1`) | `0x00000000` | Active-low button input |
| 5 | 11 (`SPI_CS2/GPIO11`) | `0x00000101` | Driven low by one mechanism entry path |
| 10 | 3 (`GPIO3`) | `0x00000101` | Driven low before PWM and high in reset path |
| 13 | 13 (`PWM1`) | `0x00000101` | 100-percent, 50-microsecond-period output |

The semantic names assigned to the original functions are reverse-engineering
hypotheses. The register addresses, table bytes, and call ordering are direct
binary evidence, but the physical purpose of every board net is not established
without electrical tracing or observation.

The firmware was corrected by defining PWM1 phase-1 output level `0x10`,
writing `0x10` explicitly during idle initialization, and writing `0x12`
explicitly when starting the motor interval. The corrected Windows TI ARM CGT
5.2.5 programming image is 1,878 bytes with SHA-256
`7B08F0E07C8FFFE43221CAFFE372912FC39BB8163B79E1617131F61591F7D406`.
Independent parsing confirmed 568 original firmware bytes, 200 `0xFF` gap
bytes, a continuous range of `0x08000000` through `0x080002FF`, and no changed
or missing original bytes. A read-only probe reported one bootloader instance,
the FlashBurner driver present, and the SPI flash present. The corrected image
was then programmed to instance zero; FlashBurner returned exit code zero and
`Programming Succeeded`. After the device was unplugged and reconnected with J7
open, the operator confirmed that pressing the eject button operated the
physical motor. This validates execution of the corrected GPIO3/PWM1 path on
the tested unit; full cartridge travel was not separately confirmed.

The TI TUSB9261 data sheet gives these package pins for a focused live test:

| U4 pin | Signal | Diagnostic purpose |
| ---: | --- | --- |
| 3 | `PWM1` | Should assert during the intended two-second motor interval |
| 9 | `GPIO1` | Should change from high to low when the eject button is pressed |
| 11 | `GPIO3` | Should change from high to low before PWM starts and return high after two seconds |
| 23 | `SPI_CS2/GPIO11` | Original output-5 net; board-level role still unknown |

On the photographed package, pin 1 is at the top-right corner and the top-edge
pins count from right to left. Use J14 round as the confirmed ground reference.
For live measurements, attach the ground lead with power removed, use an
insulated fine probe, and avoid bridging adjacent U4 pins. The first useful
measurements are U4 pin 9 unpressed/pressed, then U4 pin 11 before/during/after
a press. Pin 11 going low for two seconds would prove that button polling and
RTI timing work and would isolate the failure to PWM1 or the downstream motor
driver. Pin 11 going low and remaining low would instead indicate a stalled RTI
delay. If pin 11 never changes despite pin 9 changing, the test firmware is not
observing or acting on the input as intended.
