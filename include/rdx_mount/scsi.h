...
#define SCSI_TI_OPCODE_MAX                    0xE8
...
#define SCSI_OPENRDX_MECHANISM_STATUS         0xE8   /* Read-only RAM diagnostic. */
...
#define VPD_RDX_MEDIA_ID_PAGE_CODE                 0xC0
#define VPD_RDX_MEDIA_IDENTIFY_PAGE_CODE           0xC2
...
typedef struct _SCSI_CMD_INFO_T
{
    ...
    UINT32_T     dMediaEpoch;   /* media generation captured before CDB parsing */
    BOOLEAN_T    bFUA;    
...
/**
 * @brief Report the current PREVENT/ALLOW MEDIUM REMOVAL state.
 *
 * @return TRUE after a valid PREVENT request and FALSE after ALLOW or SCSI
 *         initialization.
 */
BOOLEAN_T scsi_medium_removal_is_prevented(void);

/**
 * @brief Clear PREVENT state for hidden-button operation mode two.
 *
 * Mode two explicitly clears the same scoped medium-removal interlock checked
 * by host LOEJ and physical-button eject requests.
 */
