 ...
 *
 * Mechanism states 1/3/6 drive this output low; stopped states use high. The
...
/**
 * @brief Preload and enable the profile-dependent active-high GPIO0 output.
 *
 * Call after identity initialization supplies the effective hardware profile.
 * Profile 38h maps GPIO0 to logical output 14 and requires high during normal
 * connected operation. Other profiles map it to logical output 11, initially
 * low. This API does not assign an electrical purpose to either mapping.
 *
...
/**
 * @brief Set logical output 11 when the selected profile maps it to GPIO0.
 *
 * Calls before profile initialization and on profile 38h have no effect:
 * GPIO0 belongs to logical output 14 on that profile and stays high here.
 *
 * @param[in] asserted TRUE for logical one/physical high, FALSE for logical
 *                     zero/physical low.
 */
