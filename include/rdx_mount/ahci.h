...
/** Keep the USB dock present while its removable SATA cartridge changes.
 *  The fixed-target setting detaches and rebuilds the whole USB device on
 *  every SATA connect-change interrupt.  On the RDX bay's empty link this can
 *  repeat indefinitely, so Windows never retains the disk PDO that Manager
 *  needs for a dock-only firmware update.  The removable-target path instead
 *  reports MEDIUM NOT PRESENT/UNIT ATTENTION through the stable LUN. */
#ifndef REMOVABLE_MEDIA_DEVICE
#define REMOVABLE_MEDIA_DEVICE   1
#endif
...
/* The asynchronous ATA completion ring contains sixteen pointer/record pairs
 * and wraps both producer and consumer indices after entry 15. Keep this
 * independent of AHCI_NCQ_DEPTH: command-slot depth and deferred-completion
 * capacity are separate runtime constraints. */
#define ATA_CALLBACK_QUEUE_DEPTH   16
...
#define PCMD_SUD_BIT     0x00000002
...
#define PTFD_STS_DF_BIT   0x00000020
#define PTFD_STS_DRQ_BIT  0x00000008
#define PTFD_STS_ERR_BIT  0x00000001
#define PTFD_STS_FAILURE_MASK  (PTFD_STS_DF_BIT | PTFD_STS_ERR_BIT)
...
#define PHY_READY_CHANGE_STATUS         ((UINT32_T)0x01 << 22)  // This interrupt is not compatible with TUSB926x PHY.  Use AHCI interface errors to determine device disconnect.
...
enum
{
    PORT_FATAL_ERROR_INTR = (TASK_FILE_ERROR_STATUS | HOST_BUS_FATAL_ERROR_STATUS |
                             HOST_BUS_DATA_ERROR_STATUS | INTERFACE_FATAL_ERROR_STATUS |
                             UNKNOWN_FIS_INTR),

    PORT_DEFAULT_INTR_ENABLE = (PORT_FATAL_ERROR_INTR | OVERFLOW_STATUS |
                                /*PHY_READY_CHANGE_STATUS |*/ PORT_CONNECT_CHANGE_STATUS |
                                SET_DEVICE_BITS_FIS_INTR | DMA_SETUP_FIS_INTR |
...
//    UINT8_T      bMultiSectorNum;   /* number of sectors transferred per interrupt */
    ...
    UINT64_T     ddMaxLBA;   /* derived from IDENTIFY DEVICE words 60-61 (28-bit) or 100-103 (48-bit) - for ATA devices only (equal to ddTrueMaxLBA unless we are emulating 4KB sectors)  */
    ...
    UINT16_T     wLogicalSectorOffset;  /* location of logical sector zero (equal to wTrueLogicalSectorOffset unless we are emulating 4KB sectors) */
    UINT16_T     wTrueLogicalSectorOffset;

    UINT32_T     wLowestAlignedLBA;    /* equal to dLogicalSectorsPerPhysicalSector - wLogicalSectorOffset (only valid when emulating 4KB logical sectors) */
    BOOLEAN_T    bLargeSectorEmulation;  /* flag indicating emulation of 4KB logical sectors for supporting HDDs greater than 2.2 TB under WinXP */
...
/** Queue deferred discovery after physical insertion if media is unready. */
...
/**
 * @brief Invalidate media and optionally reset an active locally owned slot.
 *
 * The caller completes its request locally; this recovery path never queues
 * an ATA completion callback and defers media admission to foreground service.
 *
 * @param port_num SATA port number.
 * @param reset_port TRUE to stop DMA and perform COMRESET first.
 */
