...
#define ENABLE_U1_U2_TRANSITIONS                                                                                                           \
    0 // (Default = 0) Set to 1 to enable host & device initiated transitions to U1/U2 states. Recommended to leave this disabled for maximum performance and compatibility.
#endif

#define DISABLE_U1_U2_INITIATE_WHILE_XFER_ACTIVE                                                                                           \
    1 // (Default = 1) Set to 1 to disable device-initiated U1/U2 while there is transfer activity. Fixes low throughput issue with Fresco and Intel hosts.

#define ENABLE_USB_SSC                                                                                                                     \
    0 // (Default = 0) Set to 1 to enable spread spectrum clocking. (Disabled by default for maximum compatibility with various host controllers which may have non-compliance SSC if they derive clock from motherboard)
...
#define NUM_P                                                                                                                              \
    3 // # of Rx buffers to be reported in ACK TP. (zero-based, must be less than or equal to max burst size).  Value of 3 shows best performance in 4k Write testing.
...
#define TRB_STATUS_BUFFER_SIZE_MASK                                                                                                        \
    0x00FFFFFF /* Buffer size in bytes. [Range: 0 bytes - 16 MB] Must be 0 for Status TRB and 8 for Setup TRB */
...
/* TRB Control bits */
#define TRB_CTRL_HWO_BIT                                                                                                                   \
    0x00000001 /* HW owner of descriptor (SW should set to 1, HW clears it unless Link TRB or Short packet on OUT endpt) */
...
typedef enum { USB_PM_STATE_RESUME, USB_PM_STATE_RESET, USB_PM_STATE_WAKE_UP } eUSB_PM_STATE_T;

typedef enum { USB_LOW_SPEED = 0, USB_FULL_SPEED = 1, USB_HIGH_SPEED = 2, USB_SUPER_SPEED = 3, USB_SPEED_UNKNOWN = 4 } eUSB_DEVICE_SPEED_T;
...
typedef enum { USB_PM_SUSPEND, USB_PM_RESUME, USB_PM_RESET, USB_PM_DISCONNECT } eUSB_DEVICE_PM_STATE_T;

typedef enum { USB_MSC_BOT, USB_MSC_UAS } eUSB_MS_CLASS_T;
...
UINT32_T usb_hal_get_usb_link_state(void);
