...

{
    ...
    uint32_t *spi_get_logical_pin_configuration_result;
    uint8_t *data_cursor;
    int working_result;
    ...
    operation_status = 1;
    if (condition == 0) {
        spi_get_logical_pin_configuration_result = (uint32_t *)spi_get_logical_pin_configuration(context);
        if ((char)spi_get_logical_pin_configuration_result[1] == '\0')
            goto spi_output_apply_polarity_adjusted_level;
    } else {
        spi_get_logical_pin_configuration_result = (uint32_t *)spi_get_logical_pin_configuration(context);
        if ((char)spi_get_logical_pin_configuration_result[1] != '\0')
            goto spi_output_apply_polarity_adjusted_level;
    ...
    operation_status = 0;
/* Shared target for spi output apply polarity adjusted level; incoming paths preserve the same state assumptions. */
spi_output_apply_polarity_adjusted_level:
    status_bits = *spi_get_logical_pin_configuration_result;
    if (status_bits < 8) {
...
/**
 * @brief Configure logical pin direction.
 *
 * @param context Procedure input.
 * @return No value.
 */
void spi_configure_logical_pin_direction(context) uint32_t context;

{
    /* Configure logical pin direction using only caller-provided data. */
    ...
    spi_get_logical_pin_configuration_result = (uint32_t *)spi_get_logical_pin_configuration(context & 0xff);
    sci_pio3_pointer = *spi_get_logical_pin_configuration_result;
    if (sci_pio3_pointer < 8) {
        pin_mask = 1 << (sci_pio3_pointer & 0xff);
        working_result = pin_mask;
        if (*(char *)((int)spi_get_logical_pin_configuration_result + 5) == '\0') {
            working_result = 0;
        }
        /* Snapshot the fixed-address register before decoding its status or capability fields. */
        data_cursor = (uint8_t *)&GIOOUT0_REG_OFF;
        ...
        if ((sci_pio3_pointer == 8) || (sci_pio3_pointer == 9)) {
            data_cursor = (uint8_t *)&SCI_PIO3;
            /* Configure logical pin direction; this branch selects whether its associated handler runs. */
            pin_mask = 1 << (*spi_get_logical_pin_configuration_result - 7 & 0xff);
            core_update_masked_register_bits(&SCI_PIO0, pin_mask, 0);
            operation_status = *(char *)((int)spi_get_logical_pin_configuration_result + 5);
        } else {
            if (sci_pio3_pointer != 10 && sci_pio3_pointer != 0xb)
                goto spi_output_initialize_inactive_level;
            data_cursor = (uint8_t *)&SPI_PC3;
            /* Configure logical pin direction; this branch selects whether its associated handler runs. */
            pin_mask = 2 << (*spi_get_logical_pin_configuration_result - 10 & 0xff);
...
/* Shared target for spi output initialize inactive level; incoming paths preserve the same state assumptions. */
spi_output_initialize_inactive_level:
    spi_set_logical_output(context, 0);
    return;
}

/**
 * @brief Initialize logical GPIO.
 *
 * @return No value.
 */
...
{
    /* Initialize logical GPIO; persistent state is carried in GIO pin callback table. */
    uint32_t entry_index;

    core_initialize_pwm_channel(0);
    core_initialize_pwm_channel(1);
    ...
    do {
        spi_configure_logical_pin_direction(entry_index);
...
/**
 * @brief Get logical pin configuration.
 *
 * @param entry_index SPI configuration table index.
 * @return Result produced by the procedure.
 */
uint8_t *spi_get_logical_pin_configuration(entry_index)
int entry_index;

{
    /* Get logical pin configuration; persistent state is carried in SPI transaction table. */
