...

{
    ...
    do {
        ...
        case 1:
            ...
            working_result = 0x89;
            goto ahci_port_init_wait_for_status_clear;
        case 2:
        ...
        case 3:
            ...
            goto ahci_port_init_poll_selected_status;
        case 4:
            ahci_start();
            core_atapi_device_flag = 0;
            operation_result = core_submit_ata_identify_command(0);
            break;
        case 5:
            ...
            core_parse_ata_identify_device(0);
            operation_result = 0;
            break;
        case 6:
            operation_result = 0;
            if ((1 < core_ata_sata_link_speed_generation) && (status_bits = core_get_sata_link_speed(0), status_bits < 2)) {
                ...
                ahci_port_reset();
                status_bits = core_get_sata_link_speed(0);
                if (status_bits < 2) {
                    ahci_hba_reset();
            ...
            goto ahci_port_init_commit_next_state;
        case 7:
            working_result = 0xf;
        ...
        case 9:
        case 0xb:
            operation_result = core_submit_ata_set_transfer_mode(0, 0);
            break;
        case 10:
        case 0xc:
            working_result = 1;
            poll_attempt_limit = 3000;
        /* Shared target for Ahci port init wait for status clear; incoming paths preserve the same state assumptions. */
        ahci_port_init_wait_for_status_clear:
            expected_status_bits = 0;
        /* Shared target for Ahci port init poll selected status; incoming paths preserve the same state assumptions. */
        ahci_port_init_poll_selected_status:
            operation_result = ahci_wait_complete(data_cursor, working_result, expected_status_bits, poll_attempt_limit);
            break;
        case 0xd:
            if (core_ata_dma_setup_auto_activate_supported == '\0') {
                next_dispatch_state = 0xf;
            } else {
                core_submit_ata_sata_feature_control(0, 1);
            }
            break;
        case 0xe:
    ...
    /* Shared target for ahci port init commit next state; incoming paths preserve the same state assumptions. */
    ahci_port_init_commit_next_state:
        *buffer = status_bits;
        if (status_bits == 0x10) {
            return operation_result;
        }
...

{
    ...
    if ((operation_status == 0) && (operation_status = core_issue_ahci_command_slot(0, 0, 0), operation_status == 0)) {
        poll_timeout = 9000;
...

{
    ...
    core_start_ahci_command_override_if_stopped(0);
...
{
    /* Init; persistent state is carried in AHCI ports implemented register, ahci initialized device count and related record fields. */
    uint32_t ahci_ports_implemented_register_snapshot;
    int operation_status;

    ti_memset(&ahci_initialized_device_count, 0, 0x14);
    core_register_usb_power_management_callback(&ahci_device_event_callback_entry);
    ...
    if (operation_status == 0) {
        ti_memset(&ahci_command_completion_callback, 0, 0x10);
        ahci_global_host_control_register = 2;
        ...
        if (((ahci_ports_implemented_register_snapshot & 1) != 0) && (operation_status = ahci_init_port(), operation_status == 0)) {
            ahci_initialized_device_count = ahci_initialized_device_count + 1;
        }
    }
    return ahci_initialized_device_count == 0;
}
...
{
    /* Port intr handler; persistent state is carried in AHCI port SATA error register, core ahci active command tag. */
    ...
    if ((interrupt_status_bits >> 1 & 1) != 0) {
        ...
        if (*(char *)(device_index * 0x1d0 + (uint32_t)(uint8_t)(&core_ahci_active_command_tag)[device_index * 0x1d0] + 0x800f5f8) == '\0') {
            core_queue_ata_pio_completion();
        }
    }
    if ((interrupt_status_bits >> 2 & 1) != 0) {
        core_queue_ata_dma_setup_completion(interrupt_status_bits >> 3);
    }
    if ((interrupt_status_bits >> 3 & 1) != 0) {
        core_queue_ata_set_device_bits_completion(interrupt_status_bits >> 4);
    }
    if ((interrupt_status_bits & 1) != 0) {
        /* This fixed address selects the fixed packed firmware record; its field offsets are ABI-significant. */
        if (*(char *)(device_index * 0x1d0 + (uint32_t)(uint8_t)(&core_ahci_active_command_tag)[device_index * 0x1d0] + 0x800f5f8) == '\0') {
            core_queue_ata_d2h_completion();
        }
    }
...
{
    /* Fatal error recovery; persistent state is carried in AHCI port task file data register, ahci port sata active register and related record fields. */
    uint32_t ahci_port_task_file_data_register_snapshot;
    int ahci_port_sata_active_register_snapshot;

    if (core_ata_native_command_queueing_supported == '\0') {
        ahci_port_sata_active_register_snapshot = 0;
    } else {
        ahci_port_sata_active_register_snapshot = ahci_port_sata_active_register;
    }
    ahci_stop();
    ahci_port_task_file_data_register_snapshot = ahci_port_task_file_data_register;
    if ((ahci_port_task_file_data_register_snapshot & 0x88) != 0 || ahci_port_sata_active_register_snapshot != 0) {
        ahci_port_reset();
    }
    ahci_start();
    ahci_get_TFD_info();
...
{
    /* Continue port initialization; persistent state is carried in AHCI port task file data register, core ahci port initialization state. */
    uint32_t ahci_port_task_file_data_register_snapshot;
    int ahci_run_port_initialization_state_machine_result;

    ahci_port_task_file_data_register_snapshot = ahci_port_task_file_data_register;
    if ((ahci_port_task_file_data_register_snapshot & 0x89) == 0) {
        ahci_run_port_initialization_state_machine_result = ahci_run_port_initialization_state_machine(&core_ahci_port_initialization_state);
    } else {
...
{
    /* Get TFD info; persistent state is carried in AHCI port task file data register, core ata callback write index. */
    uint32_t ahci_port_task_file_data_register_snapshot;
    uint8_t *core_ata_callback_write_index_snapshot;
    uint8_t *ahci_status_high_byte;

    /* This fixed address selects the fixed packed firmware record; its field offsets are ABI-significant. */
    core_ata_callback_write_index_snapshot = (uint8_t *)(core_ata_callback_write_index * 0xc + 0x800f521);
    /* This fixed address selects the fixed packed firmware record; its field offsets are ABI-significant. */
    ahci_status_high_byte = (uint8_t *)(core_ata_callback_write_index * 0xc + 0x800f522);
    ahci_port_task_file_data_register_snapshot = ahci_port_task_file_data_register;
    if (core_ata_callback_write_index_snapshot != (uint8_t *)0x0) {
        *core_ata_callback_write_index_snapshot = (char)ahci_port_task_file_data_register_snapshot;
    }
