...
//    CRIT("System Clock Gating: %s\n", (ENABLE_CLOCK_GATING) ? "ON" : "OFF");
    ...
    while (1)
    {
        ...
        /* GPIO2 is polled and cannot wake WFI. Keep servicing active motion
         * between RTI ticks so a short endpoint transition is not missed. */
        if (wfi_enable && !rdx_hardware_eject_in_progress())
        {
            wfi_enable = FALSE;
            asm(" wfi");  
        }
