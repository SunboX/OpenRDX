...
UINT8_T descriptor_data_buff[] =
{
    ...
    /* Serial template. The 0xCA marker is consumed by
     * send_string_descriptor() and keeps the receiver identity authoritative. */
    (2+24),
    USB_DT_STRING,
...
/**
 * @brief Apply the manufacturing unit serial to the USB string descriptor.
 *
 * The USB identity keeps its two-character prefix and appends the ten-byte
 * unit serial. The marker in the first UTF-16 code unit prevents the generic
 * descriptor response path from substituting the controller die identifier.
 *
 * @param[in] descriptor_index string-descriptor index from the device descriptor.
 *
 * @return No value.
 */
static void rdx_apply_unit_serial_to_usb_descriptor(UINT8_T descriptor_index)
{
    ...
    if (STATUS_OK != get_descriptor_ptr(
            USB_DT_STRING, &descriptor, descriptor_index))
    {
        return;
    }
    ...
    if ((identity == NULL) ||
        (descriptor[0] <
         (2U + (2U * (2U + sizeof(identity->serial))))))
    {
        return;
    }

    serial_ptr = &descriptor[2];
    serial_ptr[0] = '0';
    serial_ptr[1] = 0xCAU;
    serial_ptr[2] = '0';
    serial_ptr[3] = 0U;
    for (index = 0U; index < sizeof(identity->serial); index++)
    {
