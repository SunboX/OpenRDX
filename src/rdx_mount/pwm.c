 ...
 * Function: pwm_start
 *************************************************************************//**
 * This function starts a PWM. It can also be used to restart PWM for one-shot usage.
 *
 * @param[in] pwm_num number of the PWM [0-1].
 *                    
 * @retval None.
...
{
    /* MODE=disabled drives the inactive level immediately. A PH1D update is
     * buffered until a period boundary while continuous mode is running. */
...
void pwm_run(UINT32_T pwm_num, UINT32_T duty_cycle_percentage, UINT32_T period_us)
{
    ...
    /* Configure timing with the channel disabled: the TI PWM initialization
     * sequence loads PER/PH1D before START (SPRUFH6 section 2.5). Starting
     * first can latch the prior period, including the power-on zero value. */
    WRITE32(PWM_CFG_REG_OFF(pwm_num), PWM_CFG_RDX_IDLE);
