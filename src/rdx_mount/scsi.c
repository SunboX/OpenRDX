...
STATUS_T scsi_get_data_direction(const UINT8_T *cdb, ENDPT_DIR_T *direction)
{
    ...
    switch (cdb[0])
    {
        ...
        case SCSI_OPENRDX_MECHANISM_STATUS:
            *direction = ENDPT_DIRECTION_IN;
            break;
 ...
 *************************************************************************//**
 * Handle the TI vendor commands and the read-only OpenRDX mechanism snapshot.
 *
 * @param None.
 *
 * @retval STATUS_OK when successful.
...
inline STATUS_T scsi_handle_ti_defined_cmd(void)
{
    ...
    switch (scsi_cmd.pCmdInput->pCommandBlock[0])
    {
        ...
        case SCSI_OPENRDX_MECHANISM_STATUS:
            /* This fixed-size RAM snapshot is available with an empty bay.
             * Reserved CDB bytes cannot select addresses, writes, or motion. */
            if ((scsi_cmd.pCmdInput->pCommandBlock[1] != 0U) ||
                (scsi_cmd.pCmdInput->pCommandBlock[2] != 0U) ||
                (scsi_cmd.pCmdInput->pCommandBlock[3] != 0U) ||
                (scsi_cmd.pCmdInput->pCommandBlock[4] != 0U) ||
                (scsi_cmd.pCmdInput->pCommandBlock[5] !=
                 RDX_MECHANISM_DIAGNOSTIC_LENGTH) ||
                !rdx_mechanism_read_diagnostics(
                    (UINT8_T *)scsi_resp_buff,
                    RDX_MECHANISM_DIAGNOSTIC_LENGTH))
            {
                status = STATUS_SCSI_INVALID_CMD;
                break;
            ...
            scsi_cmd.pCmdInput->pData = (void *)scsi_resp_buff;
            scsi_cmd.pCmdInput->dDataByteCnt = RDX_MECHANISM_DIAGNOSTIC_LENGTH;
            status = STATUS_SCSI_RESPONSE_READY;
            break;
