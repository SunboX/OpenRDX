...
#define RDX_TEMPERATURE_SAMPLE_MS               10000UL
...
typedef enum _RDX_THERMAL_PHASE_T
{
    ...
    RDX_THERMAL_COOL = 1,
    RDX_THERMAL_HOT = 2,
    /* A present cartridge has no valid temperature measurement yet. */
...
static void rdx_hardware_service_cartridge_input(UINT32_T now)
{
    ...
    {
        /* Keep a debounced insertion pending until the mechanism finishes.
         * Consuming it now would skip the auxiliary output and let eject
         * completion tear down the newly inserted media. Raw candidates keep
         * their timer, and physical removal is still committed while busy. */
        if (raw_present &&
            (rdx_hardware.eject_state != RDX_EJECT_IDLE))
        {
            return;
        ...
        rdx_hardware.logical_unloaded = FALSE;
        rdx_hardware.eject_fault_latched = FALSE;
        /* SATA discovery can finish before this debounce interval. Preserve
         * its ready-time slider sample instead of relocking admitted media. */
        ...
        else
        {
            ...
            rdx_manager_clear_host_eject_policy();
            /* The bridge PHY can remain ready with the cartridge removed. */
...
/**
 * @brief Apply measured-temperature fan demand every 800 ms.
 *
 * Cooling starts only after a valid sample reaches 45 C and stops at 40 C.
 * The 41..44 C band retains the previous demand. This threshold-only policy
 * deliberately replaces a continuous baseline duty for a present cartridge.
 */
static void rdx_hardware_service_fan(UINT32_T now)
{
     ...
     * service transition. It remains disabled for the whole boot. */
    ...
    /* Unknown, cool, and absent media cannot start the fan. A failed sample
     * preserves an existing HOT phase, so lost telemetry cannot stop cooling. */
    ...
    switch (rdx_hardware.fan_state)
    {
        ...
        case RDX_FAN_STATE_STARTING:
                    ...
                    now, rdx_hardware.fan_action_deadline))
            {
                ...
                /* Hold the starting duty for a full minute before ramping. */
                rdx_hardware.fan_action_deadline =
        ...
        case RDX_FAN_STATE_RAMPING:
            /* Increase cooling by 13 percent per minute while demand remains
             * HOT. The common gate also stops an unfinished ramp at 40 C. */
            if (rdx_hardware_deadline_reached(
                    now, rdx_hardware.fan_action_deadline))
            {
...
/**
 * @brief Poll SMART at most every ten seconds without delaying an eject.
 *
 * Unknown temperature leaves a stopped fan off and preserves any active
 * cooling demand. Only a valid sample changes the hysteresis phase.
 */
static void rdx_hardware_service_temperature(UINT32_T now)
{
    ...
    if (!successful || (temperature == RDX_TEMPERATURE_UNAVAILABLE))
    {
        rdx_hardware.temperature_celsius = RDX_TEMPERATURE_UNAVAILABLE;
    ...
    else if (temperature <= RDX_TEMPERATURE_COOL_MAX)
    {
        rdx_hardware.thermal_phase = RDX_THERMAL_COOL;
    }
    /* 41..44 C retains the demand, including an unknown/off initial state. */
    rdx_hardware.temperature_deadline =
        rdx_hardware_now_ms() + RDX_TEMPERATURE_SAMPLE_MS;
}
...
static void rdx_hardware_service_media_state(UINT32_T now)
{
    ...
    if (ready != rdx_hardware.observed_ready)
    {
        ...
        {
            rdx_hardware.observed_init_failed = FALSE;
            /* Readiness is not a temperature sample. Preserve active cooling
             * across transport recovery; insertion/removal owns a fresh phase. */
            if (rdx_hardware.thermal_phase != RDX_THERMAL_HOT)
            {
                rdx_hardware.thermal_phase = RDX_THERMAL_INITIAL;
            }
            ...
            rdx_hardware.temperature_deadline = now;
...
void rdx_hardware_init(void)
{
    ...
    /* The Manager identity is available only after generic GPIO startup.
     * Apply its GPIO0 board role before the mechanism's first idle reset. */
    ...
    gio_rdx_mechanism_auxiliary_set(FALSE);
    /* Keep PWM0 off until a valid hot sample requests cooling. Profile 36h
     * retains its unconditional fan-disabled state even after a hot sample. */
    rdx_hardware_set_fan_duty(0U);
...
/** Reserve foreground disk work as soon as an eject request is pending. */
BOOLEAN_T rdx_hardware_eject_in_progress(void)
{
    return (rdx_hardware.eject_requested ||
            (rdx_hardware.eject_state != RDX_EJECT_IDLE)) ? TRUE : FALSE;
}
