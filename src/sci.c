...

{
    ...
    input_configuration = (uint32_t *)spi_get_logical_pin_configuration(input_index);
        ...
        if ((logical_input != 8) && (logical_input != 9)) {
            if ((logical_input == 10) || (logical_input == 0xb)) {
                ...
                if ((0xd < logical_input) && (logical_input < 0x17)) {
                    input_state = core_is_adc_sample_above_threshold(logical_input - 0xe & 0xff);
                }
            }
            goto sci_input_apply_configured_polarity;
        }
        input_register = (uint32_t *)&SCI_PIO2;
        logical_input = logical_input - 7;
    }
    input_state = *input_register >> (logical_input & 0xff) & 1;
/* Shared target for sci input apply configured polarity; incoming paths preserve the same state assumptions. */
sci_input_apply_configured_polarity:
    if (input_state == 0) {
...

{
    ...
    core_initialize_software_timers();
    spi_initialize_logical_gpio();
