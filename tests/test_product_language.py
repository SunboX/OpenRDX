...
def repository_paths() -> list[Path]:
    """Return tracked and pending files, including archived authored text."""

    if (PROJECT_ROOT / ".git").exists():
        result = subprocess.run(
            [
                "git", "-C", str(PROJECT_ROOT), "ls-files",
                "-z", "-co", "--exclude-standard",
            ],
            capture_output=True,
            check=True,
        )
        # Git remains authoritative for tracked files even in ignored folders.
        # NUL separators preserve names containing spaces or line breaks.
        return sorted({
            path
...
def text_paths() -> list[Path]:
    """Return readable authored text without editing verbatim license terms."""

    paths = []
    for path in repository_paths():
        if path.relative_to(PROJECT_ROOT).parts[0] == "LICENSES":
            continue
...
def violations_in_text(value: str) -> list[str]:
    ...
    violations = [
        token
        for token in tokens
        if digest(token) in DISALLOWED_TOKEN_DIGESTS
    ]
    violations.extend(
        f"{first} {second}"
...
class ProductLanguageTests(unittest.TestCase):
    ...
    def test_tracked_history_and_ignored_files_are_scanned(self):
        """Keep tracked prose in scope even when its directory is ignored."""
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            included = (
                "temp/history/design.md",
                ".superpowers/record.md",
            ...
            for name in included:
                ...
                path.write_text("OpenRDX text\n")
                subprocess.run(
                    ["git", "-C", str(root), "add", "-f", name], check=True
                )
            (root / "temp/local.md").write_text("Local working notes\n")

            with patch.dict(globals(), PROJECT_ROOT=root):
                scanned = {path.relative_to(root).as_posix() for path in text_paths()}

            self.assertTrue(set(included).issubset(scanned))
            self.assertNotIn("temp/local.md", scanned)

    def test_new_text_formats_and_pending_files_are_scanned(self):
        """Check authored text without depending on an extension allowlist."""

        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            subprocess.run(["git", "init", "-q", str(root)], check=True)
            (root / "settings.toml").write_text("title = 'OpenRDX'\n")
            (root / "NOTICE").write_text("OpenRDX attribution\n")
            (root / "image.bin").write_bytes(b"\x00\xff\x01")

            with patch.dict(globals(), PROJECT_ROOT=root):
                scanned = {path.name for path in text_paths()}

            self.assertEqual({"settings.toml", "NOTICE"}, scanned)
