...
class SataEpochAndTransferSafetyTests(unittest.TestCase):
    ...
    def test_terminal_callbacks_are_coalesced_without_skipping_readmission(self):
        ...
        # Check normal recovery separately from the earlier mechanism-active
        # deferral, which owns its own schedule/terminal-callback ordering.
        rx_recovery = function_body(
            rx_error, "if (dev_detect_status != PSSTS_DET_PHY_OFFLINE)"
        )
        schedule = re.search(
