...
MAPPING_DOCUMENT = PROJECT_ROOT / "docs" / "reference" / "led-output-map.md"
    ...
    def test_logical_output_setter_retains_active_low_polarity_branch(self):
        ...
        self.assertRegex(
            ...
            re.compile(
                ...
                r"    if \(condition == 0\) \{\n"
                r"        spi_get_logical_pin_configuration_result = "
                r"\(uint32_t \*\)spi_get_logical_pin_configuration\(context\);\n"
                r"        if \(\(char\)spi_get_logical_pin_configuration_result\[1\] == '\\0'\)\n"
                r"            goto spi_output_apply_polarity_adjusted_level;\n"
                r"    \} else \{\n"
                r"        spi_get_logical_pin_configuration_result = "
                r"\(uint32_t \*\)spi_get_logical_pin_configuration\(context\);\n"
                r"        if \(\(char\)spi_get_logical_pin_configuration_result\[1\] != '\\0'\)\n"
