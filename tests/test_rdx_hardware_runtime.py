...
class RdxHardwareRuntimeTests(unittest.TestCase):
    ...
    def test_mechanism_uses_exact_outputs_timers_and_retry_routing(self):
        """Preserve named drive phases, their values, and distinct output behavior."""
        run = function_body(
        ...
        for token in (
            ...
            "RDX_MECHANISM_NO_MEDIA_GRACE_MS        50UL",
            "RDX_MECHANISM_SUCCESS_SETTLE_MS        500UL",
            "RDX_MECHANISM_MAX_RETURN_TIMEOUTS       3U",
        ):
            self.assertIn(token, MECHANISM)
        for state, name in enumerate(
            (
                "LOW_DRIVE_CHECK_MEDIA",
                "NO_MEDIA_GRACE",
                "LOW_DRIVE_WAIT_ENDPOINT",
                "SUCCESS_SETTLE",
                "RETURN_PAUSE",
                "RECOVERY_DRIVE_WAIT_ENDPOINT",
                "RETURN_SETTLE",
                "RETRIES_EXHAUSTED",
            ),
            start=1,
        ):
            self.assertIn("RDX_MECHANISM_{} = {}".format(name, state), MECHANISM)
        self.assertIn("gio_rdx_mechanism_auxiliary_set(FALSE);", enter)
        self.assertIn("rdx_mechanism_run_motor(FALSE);", enter)
        self.assertIn("gio_rdx_motor_control_set_high(FALSE);", enter)
        self.assertIn("gio_rdx_profile_mechanism_auxiliary_set(TRUE);", enter)
        ...
        return_settle = enter[
            ...
            enter.index("case RDX_MECHANISM_RETRIES_EXHAUSTED:")
        ]
        self.assertIn("rdx_mechanism_stop_motor();", return_settle)
        self.assertNotIn("rdx_mechanism.deadline =", return_settle)
        self.assertIn("rdx_mechanism.return_drive_timeouts++;", service)
    ...
    def test_mechanism_samples_live_media_context_and_defers_boot_homing(self):
        ...
        self.assertNotIn("RDX_MECHANISM_LOW_DRIVE_CHECK_MEDIA", initialize)
        self.assertIn("if (!gio_rdx_mechanism_input_asserted())", start_homing)
        self.assertIn("RDX_MECHANISM_LOW_DRIVE_CHECK_MEDIA", start_homing)
        ...
        assert_statements_in_order(
            ...
            (
                ...
                "gio_rdx_profile_outputs_init(hardware_profile);",
    ...
    def test_eject_uses_complete_timed_return_and_retry_path(self):
        ...
        self.assertIn(
            "rdx_mechanism_enter_state(RDX_MECHANISM_LOW_DRIVE_CHECK_MEDIA, now);", start
        )
        self.assertEqual(
            3,
            service.count(
    ...
    def test_mechanism_requires_directional_gpio2_endpoint_transition(self):
        ...
        self.assertIn("!gio_rdx_mechanism_input_asserted() ? TRUE : FALSE;", reset)
        assert_statements_in_order(
            self,
            transition,
            (
    ...
    def test_temperature_and_fan_preserve_required_bands_and_cadence(self):
        """Start at 45 C, stop at 40 C, and preserve the active cooling ramp."""
        for token in (
            ...
            "RDX_TEMPERATURE_SAMPLE_MS               10000UL",
        ...
        self.assertRegex(
            temperature,
            r"if \(temperature >= RDX_TEMPERATURE_HOT_MIN\)\s*"
            r"\{\s*rdx_hardware\.thermal_phase = RDX_THERMAL_HOT;\s*\}"
            r"\s*else if \(temperature <= RDX_TEMPERATURE_COOL_MAX\)\s*"
            r"\{\s*rdx_hardware\.thermal_phase = RDX_THERMAL_COOL;\s*\}",
        )
        self.assertEqual(temperature.count("rdx_hardware.thermal_phase ="), 2)
        self.assertNotIn("hot_sample_count", HARDWARE)
        self.assertNotIn("RDX_TEMPERATURE_NORMAL_SAMPLE_MS", HARDWARE)
        ...
        self.assertNotIn("rdx_hardware.fan_action_deadline = now;", fan)
        self.assertIn(
    ...
    def test_ready_cartridge_waits_for_temperature_without_baseline_fan(self):
        """Make the first sample due on readiness while unknown media stays off."""
        initialize = function_body(HARDWARE, "void rdx_hardware_init(void)")
        media = function_body(
            HARDWARE,
            "static void rdx_hardware_service_media_state(UINT32_T now)",
        )
        self.assertIn("rdx_hardware_set_fan_duty(0U);", initialize)
        self.assertNotIn("RDX_FAN_INITIAL_DUTY", HARDWARE)
        for body in (initialize, media):
            self.assertIn("rdx_hardware.temperature_deadline = now;", body)
            self.assertNotIn("thermal_phase = RDX_THERMAL_COOL", body)
        self.assertRegex(
            media,
            r"if \(rdx_hardware\.thermal_phase != RDX_THERMAL_HOT\)\s*"
            r"\{\s*rdx_hardware\.thermal_phase = RDX_THERMAL_INITIAL;\s*\}",
        )

    def test_missing_temperature_retains_existing_cooling_demand(self):
        """Treat failed or unsupported SMART as unknown rather than hot or cool."""
        temperature = function_body(
            HARDWARE,
            "static void rdx_hardware_service_temperature(UINT32_T now)",
        )
        unavailable = temperature[
            temperature.index("if (!sata_media_smart_is_available(0U))") :
            temperature.index("if (!rdx_hardware_ata_is_idle())")
        ]
        failure = temperature[
            temperature.index("if (!successful ||") :
            temperature.index("rdx_hardware.temperature_celsius = temperature;")
        ]
        self.assertIn("temperature == RDX_TEMPERATURE_UNAVAILABLE", failure)
        for body in (unavailable, failure):
            self.assertIn("RDX_TEMPERATURE_UNAVAILABLE;", body)
            self.assertIn("return;", body)
            self.assertNotIn("thermal_phase =", body)
            self.assertNotIn("rdx_hardware_set_fan_duty", body)

    def test_temperature_cannot_start_during_pending_eject_or_owned_ata(self):
        """Check eject and ATA ownership again under the USB interrupt mask."""
        temperature = function_body(
            HARDWARE,
            "static void rdx_hardware_service_temperature(UINT32_T now)",
        )
        assert_statements_in_order(
            ...
            (
                ...
                "return;",
                "WRITE_REG32(VIM_REQMASKCLR0, RDX_USB_INTERRUPT_MASK);",
                "if (rdx_hardware_eject_in_progress() || !rdx_hardware_ata_is_idle())",
                "WRITE_REG32(VIM_REQMASKSET0, RDX_USB_INTERRUPT_MASK);",
                "return;",
                "rdx_read_smart_temperature(0U, &temperature);",
                "WRITE_REG32(VIM_REQMASKSET0, RDX_USB_INTERRUPT_MASK);",
            ),
        )
